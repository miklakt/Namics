...
\item \texttt{dynamic_engine} DynamicEngine entries. These will not be covered in this manual. If you want to use dynamic_engine (dynamic density functional calculations) contact frans.leermakers@wur.nl.
\item \texttt{clamped_engine} clamped_engine entries. Special module which uses fragments pinned at both locations. With such a tool you can model, e.g., gells. ClampedEngine entries will not be covered in this manual.
\item \texttt{tagged_engine} tagged_engine entries. Special module which uses monte carlo simulations in combination with SCF. TaggedEngine entries will not be covered in this manual.
\end{itemize}
...
You can choose the filename of the output files (that is the part before the extension) but if you do not specify the filename yourself \namics will use the 'input-filename' as the default. You can set your own filename, e.g. when you request output of a particular type: (\tt output : pro : filename : myoutputfilename) 
...
\noindent This makes only sense if you have explicitly asked for such file
 ...
 {\tt output : kal : append : false} or by (\tt output : kal : filename : myoutputfilename)
...
\item{\tt freedom} ({\tt free}/{\tt pinned}/{\tt frozen}{\tt grafted}/{\tt tagged}/{\tt clamp}) It is possible to restrict the freedom of {\tt mon}omers.  The value {\tt free} means that in principle the {\tt mon}omers can be anywhere on the lattice/system.  The value {\tt pinned} means that the {\tt mon}omer is restricted to a certain area/sub-space, defined by setting \verb=pinned_range=.  The value {\tt frozen} means that the segment is exclusively in a certain place on the lattice, defined by setting \verb=frozen_range=.  It will fully occupy the specified lattice sites and act as a solid, no iteration variables are generated, unless different internal states are defined (not implemented yet). The value of {\tt tagged} is typically used in 'tagged_engine'-module (which we do not cover in this manual yet). The value of {\tt clamp} is typically used in 'cling'-module which is not covered in the current manual yet.  
...
\item{\tt tagged\_range} Defines the range on the lattice where the segment is tagged. See section \ref{sec:ranges}. Used by 'tagged_engine' which we do not cover here in this manual yet. 
...
\item{\tt sub\_box\_size} This is used in the 'clamped_engine'-module. (we do not cover this here.
...
output : pro : filename : overwritedefaultfilenamewiththisfilename
...
\item {\tt stop\_criterion} {\tt norm\_of\_g} / {\tt max\_of\_element\_of\_|g|} (default {\tt norm\_of\_g}) The alternative for the default stopping criterion is only used in dynamic_engine. We do not cover this module. 
\end{itemize}
...
\begin{itemize}
\item {\tt gradient\_type} {\tt classical}/{\tt dynamic_engine} default {\tt classical}
...
-{\tt dynamic_engine} In this case the system searches for the best segment potentials belonging to imposed volume fraction profiles. For the time being we do not cover dynamic_engine in this manual.
\end{itemize}
...
\subsection {DynamicEngine} Written by Daniel Emmery. The model aims to predict how a system which is off-equilibrium proceeds towards equilibrium using Fickian like transport equations. This method has been pioneered by Hans Fraaije (Groningen university/Leiden university) in the 80's-90's of previous century. Example files are available how to use this module. 

\subsection {ClampedEngine: Monte Carlo routine sampling clamped fragments}  Written by Alexander Kazakov. Example files are available on how to use this module.

\subsection {TaggedEngine: Monte Carlo routine sampling tagged chains positions} Chain conformations are done on the SCF level. One segment per molecule is sampled using MC algorithm. Free energy of SCF is used in the Metropolis Algorithm.  Written by Ramanatan Varadharajan.
