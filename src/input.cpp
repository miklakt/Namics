...
void NormalizeLine(std::string& line, bool strip_tabs = true) {
...
std::filesystem::path InputDirectoryFromName(const std::string& input_name) {
...
Input::Input(const std::string& name_) {
	name=name_;
	...
	if (in_file.is_open()) {
		...
		while (in_file) {
			...
			if (In_line.length()>7 && StartsWith(In_line, "include")) {
				add = false;
				std::string filename_inc;
				const std::string include_spec = In_line.substr(8);
				...
				if (inc_file.is_open()) {
					...
					std::cout <<"'include : " << filename_inc <<"' not working because file is not found " << std::endl; Input_error=true;
				}
	...
	} else {std::cout <<  "Inputfile " << name << " is not found. " << std::endl; Input_error=true; }
...
bool Input::ReadFile(const std::string& fname, std::string &In_buffer) const {
	std::ifstream this_file;
	bool success=true;
	const std::string resolved = ResolvePath(fname);
	...
	if (this_file.is_open()) {
		...
		this_file.close();
		if (In_buffer.size()==0) {std::cout << "File " + resolved + " is empty " << std::endl; success=false; }
	} else {std::cout <<  "Inputfile " << resolved << " is not found. " << std::endl; success=false; }
	return success;
}
...
bool Input:: TestNum(std::vector<std::string> &S, const std::string& c,int num_low, int num_high, int UptoStartNumber ) const {
	int n_starts=0;
...
bool Input:: InSet(const std::vector<std::string> &Standard, const std::string& keyword) const {
	return ContainsValue(Standard, keyword);
}
bool Input:: InSet(const std::vector<std::string> &Standard, int &pos, const std::string& keyword) const {
	return ContainsValue(Standard, keyword, &pos);
}

bool Input:: InSet(const std::vector<int> &Standard, int keyword) const {
	return ContainsValue(Standard, keyword);
}
bool Input:: InSet(const std::vector<int> &Standard, int &pos, int keyword) const {
	return ContainsValue(Standard, keyword, &pos);
}

// In->CheckParameters("keyword", name, start, KEYS, PARAMETERS)
bool Input::CheckParameters(const std::string& keyword, const std::string& name, int start, const std::vector<std::string>& Standard, ParameterStore& Input) const {
	bool success=true;
	bool prop_found;
	...
	while (i<length && n_start<start){
		...
		if (set[1]=="start") {
			...
			while (k<k_length) { //remove doubles and keep last value; erase duplicates
				...
				while (l<l_length) {
					if (input_keys[k]==input_keys[l]) {
						...
						if (n_start==1) std::cout <<"Warning: " << input_keys[k] << " found twice.... " << input_values[k] << " is used!" << std::endl;
			...
			if (set[1] == keyword && set[2]== name) {
				...
				if (!prop_found) {success=false; std::cout <<"In line " << set[0] << " "  << keyword << " property '" << parameter << "' is unknown. Select from: "<< std::endl;
					for (int k=0; k<S_length; k++) std::cout << Standard[k] << std::endl;
				} else {
					...
					if (prop_found && n_found>1 && n_start==0) {success=false; std::cout <<n_start<<" "  << start << std::endl;  std::cout <<"In line " << set[0] << " " << keyword << " property '" << parameter << "' is already defined. "<< std::endl; }
					else {
						if (prop_found && n_found>1) {success=false; std::cout <<"After 'start' " << n_start << ", in line " << set[0] << " " << keyword << " property '" << parameter << "' is already defined. "<< std::endl; }
...
bool Input:: LoadItems(const std::string& template_,std::vector<std::string> &Out_key, std::vector<std::string> &Out_name, std::vector<std::string> &Out_prop) const {
NAMICS_DBG("LoadItems in Input " << std::endl); Out_key.clear();
	Out_name.clear();
	Out_prop.clear();
	bool success=true;
	for (size_t i = 0; i < elems.size() ; i++) {
		...
		if (set[1] == template_) {
			...
			auto report_unknown_name = [&](const std::vector<std::string>& shown) {
				std::cout << "In line " << set[0] << " name '" << set[3] << "' not recognised. Select from: "<< std::endl;
				PrintList(shown);
			};
			...
			if (!key_found) {std::cout<< "In line " << set[0] << " the keyword '" << set[2] << "' not recognized. Choose keywords from: " << std::endl;
				int length = KEYS.size();
				for (int k=1; k<length; k++) std::cout << KEYS[k] << std::endl;
				return false;
			}
...
bool Input:: CheckInput(void) {
	...
	if (elems.empty()) {std::cout << "inputfile is empty " << std::endl; success=false; }
	for (const std::string& entry : elems) {
		last_start=false;
		std::vector<std::string> set;
		split(entry,':',set);
		if (set.size() !=5) {if (set[1]!="start") {
			...
			std::cout << " Line number " << set[0] << " does not contain 4 items" << std::endl; success=false; return false;}
		}
	...
	for (size_t i = 0; success && i < elems.size(); ++i) {
		...
		if (set[1]=="output") {
			if (set[2] != "json") {
				std::cout << "Value for output extension '" << set[2] << "' not allowed. Only 'json' is supported." << std::endl;
				success=false;
			} else {
	...
	for (size_t i = 0; i < elems.size(); ++i) {
		std::vector<std::string> set;
		split(elems[i],':',set);
		const std::string& word=set[1];
		if (!InSet(KEYS, word)) {std::cout << word << " is not valid keyword in line " << set[0] << std::endl;
			std::cout << "select one of the following:" << std::endl;
			for( int k=0; k<key_length; k++) std::cout << KEYS[k] << std::endl;
			success=false;
		}
	}
...
bool Input::MakeLists(int start) {
	...
	auto test_count = [&](std::vector<std::string>& list, const std::string& key, int low, int high, const std::string& error, bool fail_on_error = true) {
...
void Input::parseOutputInfo() {
	for (const std::string &line : elems) {
